...
def auth():
    ...
    if(is_successful):
        session["user"] = user
        session['is_successful'] = is_successful
        return redirect('/')
    else:
        session['is_successful'] = is_successful
...
@app.route('/addtocart')
def addtocart():
    #coming from productdetails.html
    code = request.args.get('code', '')
    ...
    session["cart"]=cart
    return redirect(url_for('cart', code=code, product=product))
...
def updatecart():
    ...
    if update == '+':
        cart[code]['qty'] = int(qty) +1
    else:
        cart[code]['qty'] = int(qty) -1

    cart[code]['subtotal'] = cart[code]['qty']*product[code]['unit_price']    
